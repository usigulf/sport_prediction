...
| **9** | [Live in-play ML](#9-live-in-play-ml) | ✅ You confirmed done (build 24, VPS cron) | — |
...
*(Build 24 / TestFlight + VPS live cron — commit `706cbe9`.)*

**v0:** Score-adjusted re-runs of the pre-game model (`inplay_v0` tag), 2-min server cron, 60s mobile polling.

**Future (optional):** Dedicated in-play model, `live_predictions` time series, mid-game swing push alerts.

---

## Roadmap complete

All nine items are shipped and confirmed. Next priorities are product/ops (App Store release, marketing, season data) — not tracked on this list.
